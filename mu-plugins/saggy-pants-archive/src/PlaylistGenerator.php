...
class PlaylistGenerator {
	...
	private array $tracks             = array();
	private array $compilationPostIds = array( '6041', '4533' );
	private array $noDashArtists      = array( 'lucky bullet', 'mawda', 'moth', 'swound', 'the starrlings', 'thisisthewaytheworldends' );
	...
	private function parse( string $html ): void {
		...
		foreach ( $posts as $post ) {
			...
			foreach ( $audioElements as $audioEl ) {
				...
				if ( $figcaption ) {
					$caption = $this->clean( $figcaption->textContent );
				}
	...
	private function parseCompTrack( string $t ): array {
		...
		$parts = explode( ' - ', $t, 2 );
		if ( count( $parts ) === 2 ) {
			return array( $this->clean( $parts[0] ), $this->clean( $parts[1] ) );
		}
		$parts = explode( '-', $t, 2 );
		if ( count( $parts ) === 2 ) {
			return array( $this->clean( $parts[0] ), $this->clean( $parts[1] ) );
		}
		return array( 'Unknown', $this->clean( $t ) );
	}

	private function fixTrack( string $t, string $artist ): string {
		if ( stripos( $t, 'Axminster' ) !== false && stripos( $t, 'Yes Car Credit' ) !== false ) {
			$t = 'Yes Car Credit';
		}
		if ( in_array( strtolower( $artist ), $this->noDashArtists ) ) {
	...
	public function getJukeboxBlock(): string {
		return $this->getJukeboxBlockWithAttrs( array() );
	}
